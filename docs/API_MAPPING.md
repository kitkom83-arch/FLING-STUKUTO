...
Status: ORO-2B fail-closed callback stub only for OroPlay API / Seamless Wallet integration. These rows add preferred staging route skeletons only and do not add callback processing, services, migrations, deploy, production DB access, real money runtime flow, live payout, live provider calls, callback wallet mutation, runtime wallet mutation, runtime ledger mutation, or hardcoded secrets.
...
| Callback balance endpoint | `/api/oroplay/balance` | POST | ORO-2B fail-closed stub only | Seamless Wallet callback planning | ORO-2B fail-closed stub only; no production DB; no real money; no wallet mutation; no ledger mutation; no live OroPlay API; no external network |
| Callback transaction endpoint | `/api/oroplay/transaction` | POST | ORO-2B fail-closed stub only | Seamless Wallet callback planning | ORO-2B fail-closed stub only; no production DB; no real money; no wallet mutation; no ledger mutation; no auto-credit; no live OroPlay API; ledger/idempotency/reconciliation required before future mutation |
| Optional callback balance alias | `/api/balance` | POST | ORO-2B optional/provider-required-only disabled | Seamless Wallet callback planning | Disabled in ORO-2B; provider-required-only; no production DB; no real money; no wallet mutation; no ledger mutation; must share guarded internal handler later |
| Optional callback transaction alias | `/api/transaction` | POST | ORO-2B optional/provider-required-only disabled | Seamless Wallet callback planning | Disabled in ORO-2B; provider-required-only; no production DB; no real money; no wallet mutation; no ledger mutation; no auto-credit; must share guarded internal handler later |
