...
Status: ORO-2B fail-closed callback stub only remains the active runtime behavior, and ORO-2C readiness contract only is current for OroPlay API / Seamless Wallet integration. These rows are not production runtime and do not add callback processing, services, migrations, deploy, production DB access, real money runtime flow, live payout, live provider calls, callback wallet mutation, runtime wallet mutation, runtime ledger mutation, Prisma write, or hardcoded secrets.
...
| Callback balance endpoint | `/api/oroplay/balance` | POST | ORO-2B fail-closed route exists; ORO-2C readiness contract only | Seamless Wallet callback planning | Not production runtime; no production DB; no real money; no wallet mutation; no ledger mutation; no live OroPlay API; no external network |
| Callback transaction endpoint | `/api/oroplay/transaction` | POST | ORO-2B fail-closed route exists; ORO-2C readiness contract only | Seamless Wallet callback planning | Not production runtime; no production DB; no real money; no wallet mutation; no ledger mutation; no auto-credit; no live OroPlay API; ledger/idempotency/reconciliation required before future mutation |
| Optional callback balance alias | `/api/balance` | POST | ORO-2C readiness contract only; alias not enabled | Seamless Wallet callback planning | Disabled in ORO-2C; provider-required-only future decision; no production DB; no real money; no wallet mutation; no ledger mutation; must share guarded internal handler later |
| Optional callback transaction alias | `/api/transaction` | POST | ORO-2C readiness contract only; alias not enabled | Seamless Wallet callback planning | Disabled in ORO-2C; provider-required-only future decision; no production DB; no real money; no wallet mutation; no ledger mutation; no auto-credit; must share guarded internal handler later |
