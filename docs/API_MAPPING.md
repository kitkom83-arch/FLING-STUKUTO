...
| OroPlay diagnostics | Auth readiness pack | Sanitized createtoken diagnostic and optional read-only balance probe | docs/static script | `src/staging-scripts/oroplayAuthDiagnostic.js`, `src/local-smoke-tests/oroplayAuthDiagnosticSanitizerSmoke.js`, `docs/OROPLAY_LIVE_READINESS.md` | `OROPLAY_BASE_URL`, `OROPLAY_CLIENT_ID`, `OROPLAY_CLIENT_SECRET`, optional `OROPLAY_DIAGNOSTIC_BALANCE=1` | sanitized auth/balance body with token/accessToken/Bearer redacted | staging operator diagnostics | none | docs/static verification | Do not enable `OROPLAY_ENABLED=1` until the auth diagnostic returns 200 and balance read-only smoke passes. |
