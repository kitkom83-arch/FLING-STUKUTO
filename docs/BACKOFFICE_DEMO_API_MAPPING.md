...
| Promotions | Configure promotions, calculator, preview, bonus/free-credit copy, turnover/withdraw condition copy, and admin promotion management | Current real backend is limited to member-facing `GET /api/promotions`, `POST /api/promotions/:id/claim`; relationship reads use `GET /api/admin/reports/summary`, `GET /api/admin/reports/wallet-ledger`, member wallet ledger, and member history promotion/turnover rows | `promotion.controller`, `promotion.service`, `report.service`, `member.service` | `src/money-demo-ui/admin.html`, `src/money-demo-ui/member.html`, `src/money-demo-ui/app.js`, `src/admin-ui/app.js` markers | `PARTIAL` | BACKOFFICE-PROMOTION-BONUS-CONNECT-23 connects promotion campaign and bonus/condition visibility to existing backend routes only. Member/admin UI uses `GET /api/promotions` and read-only ledger/report relationship data. `POST /api/promotions/:id/claim` exists but is not surfaced in these backoffice demo UIs because it can create a promotion_bonus ledger row and turnover requirement. Admin promotion CRUD/config remains `MISSING_BACKEND`; claim execution remains guarded/out-of-scope for this read-only UI phase. | Design and implement a guarded admin promotion config API/DB phase separately. Keep claim/bonus credit runtime actions behind explicit approval and local-safe guards. |
