...
| Promotions | Configure promotions, calculator, preview, bonus/free-credit copy, turnover/withdraw condition copy, and admin promotion management | Current real backend is limited to member-facing `GET /api/promotions`, `POST /api/promotions/:id/claim`; relationship reads use `GET /api/admin/reports/summary`, `GET /api/admin/reports/wallet-ledger`, member wallet ledger, and member history promotion/turnover rows | `promotion.controller`, `promotion.service`, `report.service`, `member.service` | `src/money-demo-ui/admin.html`, `src/money-demo-ui/member.html`, `src/money-demo-ui/app.js`, `src/admin-ui/app.js` markers | `PARTIAL` | Promotion visibility is connected through `GET /api/promotions`. BACKOFFICE-PROMOTION-BONUS-CONNECT-23 connects promotion campaign and bonus/condition visibility to existing backend routes only. BACKOFFICE-PROMOTION-CLAIM-GUARD-24 marks promotion claim action as guarded hold / `OUT_OF_SCOPE_NOW` for UI runtime: member/admin UI shows local-safe preflight only, guarded hold, and fail-closed copy. `POST /api/promotions/:id/claim` exists but is not surfaced as an executable frontend action because it can create a promotion_bonus ledger row and turnover requirement. Bonus ledger/turnover remain visibility only. Admin promotion CRUD/config remains `MISSING_BACKEND`. | Design and implement a guarded admin promotion config API/DB phase separately. Keep claim/bonus credit runtime actions behind explicit approval and local-safe guards. |
