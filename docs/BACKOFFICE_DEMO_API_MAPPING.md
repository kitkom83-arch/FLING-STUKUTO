...
| Promotions | Configure promotions, calculator, preview, bonus/free-credit copy, turnover/withdraw condition copy, and admin promotion management | Member-facing `GET /api/promotions`, guarded `POST /api/promotions/:id/claim`, admin read-only config visibility `GET /api/admin/promotions`; relationship reads use `GET /api/admin/reports/summary`, `GET /api/admin/reports/wallet-ledger`, member wallet ledger, and member history promotion/turnover rows | `promotion.controller`, `promotion.service`, `admin.controller`, `report.service`, `member.service` | `src/money-demo-ui/admin.html`, `src/money-demo-ui/member.html`, `src/money-demo-ui/app.js`, `src/admin-ui/app.js` markers | `PARTIAL` | Promotion visibility is connected through `GET /api/promotions`. BACKOFFICE-PROMOTION-ADMIN-CONFIG-25 adds guarded admin promotion config read-only/local-safe visibility through `GET /api/admin/promotions` using the existing `Promotion` model only. Create/update/delete admin promotion config remains out of scope. BACKOFFICE-PROMOTION-CLAIM-GUARD-24 keeps promotion claim action on guarded hold / `OUT_OF_SCOPE_NOW` for UI runtime: member/admin UI shows local-safe preflight only, guarded hold, and fail-closed copy. `POST /api/promotions/:id/claim` exists but is not surfaced as an executable frontend action because it can create a promotion_bonus ledger row and turnover requirement. Bonus ledger/turnover remain visibility only. New project.zip is a static UI/mock reference only, not a backend source. | Expand admin promotion config only in a separately approved guarded CRUD phase. Keep claim/bonus credit runtime actions behind explicit approval and local-safe guards. |
