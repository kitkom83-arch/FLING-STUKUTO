...
| PATCH | `/admin/admins/:id/role` | Admin | `admin.manage` | `role`, required `reason`, optional `permissions` array or `null` | assigned admin role and effective permissions | `400`, `401`, `403`, `404` | `admin.role.update`; audit metadata includes actor, target role, reason, before/after, and site code |
