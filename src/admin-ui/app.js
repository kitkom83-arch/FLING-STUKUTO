(function () {
  ...
  const PROMOTION_BONUS_CONNECTED_NOTE =
    "Promotion/bonus visibility stays backend-connected/read-only/local-safe through GET /api/promotions, GET /api/admin/promotions, GET /api/admin/reports/summary, GET /api/admin/reports/wallet-ledger, and existing member history promotion claim/turnover rows; GET /api/admin/promotions is admin promotion config permission guarded route guard evidence for adminAuth, siteAccess, and settings.promotion.view. POST /api/promotions/:id/claim is not surfaced in backoffice because it can create promotion_bonus ledger and turnover requirement rows. Promotion claim guard is local-safe preflight only, guarded hold, and fail-closed until a future approved backend guard opens runtime. Admin promotion config audit boundary keeps no runtime credit action and no create/update/delete promotion actions.";
  const PROMOTION_ADMIN_WRITE_READINESS_NOTE =
  ...
  const PROMOTION_ADMIN_DRY_RUN_API_STUB_NOTE =
    "promotion admin dry-run API stub is pure stub, contract-only, not mounted, validate-only, no DB write, no runtime credit action, no ledger creation, no turnover creation, and write locked.";
  const PROMOTION_ADMIN_DRY_RUN_UI_PREVIEW_NOTE =
  ...
  const PROMOTION_ADMIN_DRY_RUN_SERVICE_READINESS_NOTE =
  ...
  const PROMOTION_ADMIN_DRY_RUN_AUDIT_LEDGER_READINESS_NOTE =
  ...
  const PROMOTION_ADMIN_DRY_RUN_RUNTIME_PREFLIGHT_NOTE =
  ...
  const PROMOTION_ADMIN_DRY_RUN_MOUNT_AUTHORIZATION_NOTE =
  ...
  const PROMOTION_ADMIN_DRY_RUN_ENABLEMENT_RUNBOOK_NOTE =
  ...
  const PROMOTION_ADMIN_DRY_RUN_HOUSEKEEPING_NOTE =
