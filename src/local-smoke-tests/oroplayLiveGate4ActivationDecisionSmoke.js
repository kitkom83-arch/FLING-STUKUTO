...
const SMOKE_FILE = "src/local-smoke-tests/oroplayLiveGate4ActivationDecisionSmoke.js";
...
function assertNoSecretLiteralValues(label, text) {
  const isSmokeFile = /oroplayLiveGate4ActivationDecisionSmoke\.js$/.test(label);
  const scanned =
    label === "package.json"
...
function assertNoLiveClaim(label, text) {
  ...
  const forbidden = [
    ["gate 4", "is live"].join(" "),
    ["oroplay-live-gate-4", "is live"].join(" "),
...
function assertLiveReadinessDoc(text) {
  assertIncludes("Live readiness doc", text, [
    ...
    "ORO-LIVE-GATE-4 is a runtime activation decision gate only.",
    "### Gate Prerequisites",
...
function assertIntegrationPlanDoc(text) {
  assertIncludes("Integration plan doc", text, [
    ...
    "ORO-LIVE-GATE-1 closed/pass. ORO-LIVE-GATE-2 closed/pass. ORO-LIVE-GATE-3 closed/pass. ORO-LIVE-GATE-4 current.",
    "ORO-LIVE-GATE-4 is a runtime activation decision gate only.",
    "Gate 5 only may act as the actual controlled activation gate if Gate 4 approves.",
  ]);
}

function assertSmokeCoverageDoc(text) {
  assertIncludes("Smoke coverage doc", text, [
    "`oroplayLiveGate4ActivationDecisionSmoke.js`",
    "`npm run smoke:oroplay-live-gate-4`",
...
function assertPackageRegistration() {
  ...
  assert.strictEqual(
    pkg.scripts["smoke:oroplay-live-gate-4"],
    "node src/local-smoke-tests/oroplayLiveGate4ActivationDecisionSmoke.js",
    "package.json missing ORO-LIVE-GATE-4 smoke script."
  );
}
...
function main() {
  ...
  console.log("ORO-LIVE-GATE-4 live readiness doc contract: PASS");

  assertCurrentStatusDoc(currentStatus);
  console.log("ORO-LIVE-GATE-4 current status contract: PASS");

  assertIntegrationPlanDoc(integrationPlan);
  console.log("ORO-LIVE-GATE-4 integration plan contract: PASS");

  assertSmokeCoverageDoc(smokeCoverage);
  console.log("ORO-LIVE-GATE-4 smoke coverage contract: PASS");

  assertPackageRegistration();
  console.log("ORO-LIVE-GATE-4 package registration: PASS");

  assertStaticScans();
  console.log("ORO-LIVE-GATE-4 static safety scan: PASS");

  console.log("ORO-LIVE-GATE-4 runtime activation decision smoke: PASS");
}
...
if (require.main === module) {
  try {
    ...
    console.error("ORO-LIVE-GATE-4 runtime activation decision smoke: FAIL");
