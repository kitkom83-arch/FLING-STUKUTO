...
const UI_MARKER = "BACKOFFICE-PROMOTION-ADMIN-DRY-RUN-REVIEW-PACKET-FINGERPRINT-53";
...
function assertFingerprintMarkers(packet) {
  assert.strictEqual(packet.fingerprintSourceVersion, "promotion-dry-run-review-packet-v1");
  assert(Object.prototype.hasOwnProperty.call(packet, "packetFingerprint"));
  assert(Object.prototype.hasOwnProperty.call(packet, "payloadFingerprint"));
  assert(Object.prototype.hasOwnProperty.call(packet, "diffFingerprint"));
  assert(Object.prototype.hasOwnProperty.call(packet, "safetyFingerprint"));
  assert(Object.prototype.hasOwnProperty.call(packet, "fingerprintStatus"));
}

function assertFreshFingerprint(packet) {
  assertFingerprintMarkers(packet);
  assert.strictEqual(packet.fingerprintStatus, "fresh");
  assert(packet.packetFingerprint, "fresh packetFingerprint missing");
  assert(packet.payloadFingerprint, "fresh payloadFingerprint missing");
  assert(packet.diffFingerprint, "fresh diffFingerprint missing");
  assert(packet.safetyFingerprint, "fresh safetyFingerprint missing");
}

function assertInvalidFingerprint(packet) {
  assertFingerprintMarkers(packet);
  assert.strictEqual(packet.fingerprintStatus, "stale");
  assert(packet.packetFingerprint, "stale packetFingerprint missing");
  assert(packet.payloadFingerprint, "stale payloadFingerprint missing");
  assert(packet.diffFingerprint, "stale diffFingerprint missing");
  assert(packet.safetyFingerprint, "stale safetyFingerprint missing");
}

function assertUnavailableFingerprint(packet) {
  assertFingerprintMarkers(packet);
  assert.strictEqual(packet.fingerprintStatus, "unavailable");
  assert.strictEqual(packet.packetFingerprint, null);
  assert.strictEqual(packet.payloadFingerprint, null);
  assert.strictEqual(packet.diffFingerprint, null);
  assert.strictEqual(packet.safetyFingerprint, null);
}
...
function assertStalePacket(packet) {
  ...
  assertInvalidFingerprint(packet);
  assertSafetyFlags(packet);
}
...
async function main() {
  ...
  assertIncludes("admin.html", adminHtml, [
    ...
    "admin-promotion-dry-run-review-packet-fingerprint-status",
    "admin-promotion-dry-run-review-packet-fingerprint",
    "admin-promotion-dry-run-review-packet-payload-fingerprint",
    "admin-promotion-dry-run-review-packet-diff-fingerprint",
    "admin-promotion-dry-run-review-packet-safety-fingerprint",
    ...
    "Fingerprint status is unavailable until a successful dry-run creates a review packet.",
  ...
  assertIncludes("package.json", packageJson, [
    "smoke:backoffice-promotion-admin-dry-run-review-packet-fingerprint",
    "backofficePromotionAdminDryRunReviewPacketFingerprintSmoke.js",
  ]);
  ...
  const fingerprintState = getElement(harness, "admin-promotion-dry-run-review-packet-fingerprint-state");
  const fingerprintStatus = getElement(harness, "admin-promotion-dry-run-review-packet-fingerprint-status");
  const packetFingerprintValue = getElement(harness, "admin-promotion-dry-run-review-packet-fingerprint");
  const payloadFingerprintValue = getElement(harness, "admin-promotion-dry-run-review-packet-payload-fingerprint");
  const diffFingerprintValue = getElement(harness, "admin-promotion-dry-run-review-packet-diff-fingerprint");
  const safetyFingerprintValue = getElement(harness, "admin-promotion-dry-run-review-packet-safety-fingerprint");
  const fingerprintSourceVersionValue = getElement(harness, "admin-promotion-dry-run-review-packet-fingerprint-source-version");
  ...
  assert.strictEqual(fingerprintStatus.textContent, "fresh");
  assert.strictEqual(fingerprintState.textContent.includes("fresh"), true);
  assert.strictEqual(fingerprintSourceVersionValue.textContent, "promotion-dry-run-review-packet-v1");
  let packet = parsePacket(harness);
  assertReadyPacket(packet);
  const firstFingerprintSnapshot = {
    packetFingerprint: packet.packetFingerprint,
    payloadFingerprint: packet.payloadFingerprint,
    diffFingerprint: packet.diffFingerprint,
    safetyFingerprint: packet.safetyFingerprint,
  };
  const validResponse = harness.routeResponses[harness.routeResponses.length - 1];
  assertCommonResponseFlags(validResponse);

  const callsBeforeDeterministicRun = dryRunCallCount(harness.fetchCalls);
  await invokePrimaryClick(submitButton);
  await waitFor(() => dryRunCallCount(harness.fetchCalls) === callsBeforeDeterministicRun + 1, "deterministic dry-run repeat");
  await waitFor(() => validationStatus.textContent === "validated", "deterministic validation");
  packet = parsePacket(harness);
  assertReadyPacket(packet);
  assert.strictEqual(packet.packetFingerprint, firstFingerprintSnapshot.packetFingerprint);
  assert.strictEqual(packet.payloadFingerprint, firstFingerprintSnapshot.payloadFingerprint);
  assert.strictEqual(packet.diffFingerprint, firstFingerprintSnapshot.diffFingerprint);
  assert.strictEqual(packet.safetyFingerprint, firstFingerprintSnapshot.safetyFingerprint);
  ...
  assert.strictEqual(fingerprintStatus.textContent, "stale");
  assert.strictEqual(fingerprintState.textContent.includes("stale/invalid"), true);
  packet = parsePacket(harness);
  ...
  assert.strictEqual(packet.isStale, false);
  assertUnavailableFingerprint(packet);
  assert.strictEqual(fingerprintStatus.textContent, "unavailable");
  assert.strictEqual(fingerprintState.textContent.includes("unavailable"), true);
  ...
  await invokePrimaryClick(selectFirstButton);
  await waitFor(() => getElement(harness, "admin-promotion-dry-run-promotion-id").value === "local-smoke-promo-52-a", "reselect first promotion");
  setValidForm(harness);
  setValue(harness, "admin-promotion-dry-run-title", "Summer Bonus A Review Packet Fingerprint");
  const callsBeforeFingerprintChange = dryRunCallCount(harness.fetchCalls);
  await invokePrimaryClick(submitButton);
  await waitFor(() => dryRunCallCount(harness.fetchCalls) === callsBeforeFingerprintChange + 1, "fingerprint change dry-run");
  await waitFor(() => freshnessValue.textContent === "fresh", "fresh packet after fingerprint change");
  packet = parsePacket(harness);
  assert.strictEqual(packet.status, "ready_for_review");
  assert.strictEqual(packet.reason, "dry_run_completed");
  assert.strictEqual(packet.freshness, "fresh");
  assert.strictEqual(packet.isStale, false);
  assert.strictEqual(packet.promotionId, "local-smoke-promo-52-a");
  assert(packet.originalSnapshot, "packet originalSnapshot missing after fingerprint change");
  assert.strictEqual(packet.originalSnapshot.title, "Summer Bonus A");
  assert(packet.formPayload, "packet formPayload missing after fingerprint change");
  assert.strictEqual(packet.formPayload.after.title, "Summer Bonus A Review Packet Fingerprint");
  assert.strictEqual(packet.formPayload.after.bonusValue, 300);
  assert.strictEqual(packet.formPayload.after.maxWithdraw, 1200);
  assert.strictEqual(packet.auditReason, "stale packet smoke");
  assert.strictEqual(packet.riskAcknowledgement, true);
  ...
  assertFreshFingerprint(packet);
  assertSafetyFlags(packet);
  assert.notStrictEqual(packet.packetFingerprint, firstFingerprintSnapshot.packetFingerprint);
  assert.notStrictEqual(packet.payloadFingerprint, firstFingerprintSnapshot.payloadFingerprint);
  assert.notStrictEqual(packet.diffFingerprint, firstFingerprintSnapshot.diffFingerprint);
  assert.strictEqual(packet.safetyFingerprint, firstFingerprintSnapshot.safetyFingerprint);
  ...
  assertUnavailableFingerprint(noSelectionPacket);
  assert.strictEqual(noSelectionFingerprintStatus.textContent, "unavailable");
  ...
  const preflightFingerprintStatus = getElement(preflightHarness, "admin-promotion-dry-run-review-packet-fingerprint-status");
  ...
  assertUnavailableFingerprint(preflightPacket);
  assert.strictEqual(preflightFingerprintStatus.textContent, "unavailable");
  ...
  console.log("Backoffice promotion admin dry-run review packet fingerprint source contract: PASS");
  console.log("Backoffice promotion admin dry-run review packet fingerprint fresh packet generation: PASS");
  console.log("Backoffice promotion admin dry-run review packet fingerprint deterministic checksum: PASS");
  console.log("Backoffice promotion admin dry-run review packet fingerprint payload change detection: PASS");
  console.log("Backoffice promotion admin dry-run review packet fingerprint stale blocking: PASS");
  console.log("Backoffice promotion admin dry-run review packet fingerprint no-selection fail-closed: PASS");
  console.log("Backoffice promotion admin dry-run review packet fingerprint client preflight blocking: PASS");
  console.log("Backoffice promotion admin dry-run review packet fingerprint safety flags: PASS");
  console.log("Backoffice promotion admin dry-run review packet fingerprint secret scan: PASS");
  console.log("Backoffice promotion admin dry-run review packet fingerprint smoke: PASS");
}

main().catch((error) => {
  console.error("Backoffice promotion admin dry-run review packet fingerprint smoke: FAIL");
  console.error(error);
  process.exitCode = 1;
});
