...
const STATIC_SCAN_FILES = [
  ...
  SMOKE_COVERAGE_DOC,
  PACKAGE_JSON,
  "src/local-smoke-tests/oroplayLiveGate2CanaryPlanSmoke.js",
];
...
function assertNoSecretLiteralValues(label, text) {
  const isSmokeFile = /oroplayLiveGate2CanaryPlanSmoke\.js$/.test(label);
  const scanned =
    label === "package.json"
...
function assertNoLiveClaim(label, text) {
  ...
  const forbidden = [
    ["gate 2", "is live"].join(" "),
    ["oroplay-live-gate-2", "is live"].join(" "),
...
function assertLiveReadinessDoc(text) {
  assertIncludes("Live readiness doc", text, [
    ...
    "ORO-LIVE-GATE-2 is a read-only controlled canary plan gate only.",
    "### Preflight Checklist",
...
function assertIntegrationPlanDoc(text) {
  assertIncludes("Integration plan doc", text, [
    ...
    "ORO-LIVE-GATE-1 closed/pass. ORO-LIVE-GATE-2 current.",
    "ORO-LIVE-GATE-2 is a read-only controlled canary plan gate only.",
...
function assertSmokeCoverageDoc(text) {
  assertIncludes("Smoke coverage doc", text, [
    "`oroplayLiveGate2CanaryPlanSmoke.js`",
    "`npm run smoke:oroplay-live-gate-2`",
...
function assertPackageRegistration() {
  ...
  assert.strictEqual(
    pkg.scripts["smoke:oroplay-live-gate-2"],
    "node src/local-smoke-tests/oroplayLiveGate2CanaryPlanSmoke.js",
    "package.json missing ORO-LIVE-GATE-2 smoke script."
  );
}
...
function main() {
  ...
  console.log("ORO-LIVE-GATE-2 live readiness doc contract: PASS");

  assertCurrentStatusDoc(currentStatus);
  console.log("ORO-LIVE-GATE-2 current status contract: PASS");

  assertIntegrationPlanDoc(integrationPlan);
  console.log("ORO-LIVE-GATE-2 integration plan contract: PASS");

  assertSmokeCoverageDoc(smokeCoverage);
  console.log("ORO-LIVE-GATE-2 smoke coverage contract: PASS");

  assertPackageRegistration();
  console.log("ORO-LIVE-GATE-2 package registration: PASS");

  assertStaticScans();
  console.log("ORO-LIVE-GATE-2 static safety scan: PASS");

  console.log("ORO-LIVE-GATE-2 read-only controlled canary plan smoke: PASS");
}
...
if (require.main === module) {
  try {
    ...
    console.error("ORO-LIVE-GATE-2 read-only controlled canary plan smoke: FAIL");
