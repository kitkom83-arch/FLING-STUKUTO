...
const helper = require("../game-provider-mock/oro10hApprovalChainRolloverReviewDecisionFinalizationBoundary");
const fixtures = require("../game-provider-mock/oro10hApprovalChainRolloverReviewDecisionFinalizationBoundaryFixtures");
...
const BOUNDARY = "src/game-provider-mock/oro10hApprovalChainRolloverReviewDecisionFinalizationBoundary.js";
const FIXTURES = "src/game-provider-mock/oro10hApprovalChainRolloverReviewDecisionFinalizationBoundaryFixtures.js";
const SMOKE = "src/local-smoke-tests/oro10hApprovalChainRolloverReviewDecisionFinalizationBoundarySmoke.js";
const WRAPPER = "src/local-smoke-tests/oro10hSmoke.js";
const SCRIPT = "smoke:oro-10h";
const DETAIL_SCRIPT = "smoke:oro-10h:detailed";
const PASS_MESSAGE = "ORO-10H approval chain rollover review decision finalization boundary smoke: PASS";
const FAIL_MESSAGE = "ORO-10H approval chain rollover review decision finalization boundary smoke: FAIL";
...
function assertNoLongOro10hFilenames() {
  const longPattern =
    /ORO_10H_.*(APPROVAL_CHAIN_ROLLOVER_REVIEW_DECISION_FINALIZATION_BOUNDARY_FINALIZATION|FINALIZATION_FINALIZATION|REVIEW_REVIEW|DECISION_DECISION)/;
...
function assertDocsAndRegistration() {
  for (const file of [DOC_10H, BOUNDARY, FIXTURES, SMOKE, WRAPPER]) readRequired(file);
  ...
  for (const marker of [
    ...
    "ORO-10G closed.",
    "ORO-10H current.",
    "review decision finalization boundary only",
    "finalization in ORO-10H is a static/mock record only",
    "finalization result is not runtime authorization",
    "finalization result is not signed runtime approval",
    "finalization result is not a decision that authorizes route/runtime",
  ...
  for (const marker of [BOUNDARY, FIXTURES, SMOKE, WRAPPER, DOC_10H, SCRIPT, DETAIL_SCRIPT, "oro10h"]) {
  ...
  for (const [file, markers] of [
    ...
    [
      ...
      ["## ORO-10H Current", "ORO-10G closed.", "ORO-10H current.", ORO10H_SCOPE],
    ],
    [
...
function assertStaticSafety() {
  ...
  for (const marker of unsafeRuntimePatterns) assert(!text.includes(marker), `ORO-10H files must not contain ${marker}.`);
  for (const file of [BOUNDARY, FIXTURES, SMOKE, WRAPPER, DOC_10H]) assertNoSensitiveShape(file, readRequired(file));
}
...
function assertHappyPath(summary) {
  ...
  assert.strictEqual(summary.phase, ORO10H_PHASE);
  assert.strictEqual(summary.result, PASS);
  assert.strictEqual(summary[ORO10H_RESULT_KEY], PASS);
  assert.strictEqual(summary[DEPENDS_ON_ORO10G_KEY], true);
  assert.strictEqual(summary[ORO10G_PASSED_KEY], true);
  assert.strictEqual(summary[VERIFIED_ORO10G_ONLY_KEY], true);
  ...
  assert.strictEqual(summary.oro10gClosed, true);
  assert.strictEqual(summary.reviewDecisionFinalizationBoundaryScope, ORO10H_SCOPE);
  assert.strictEqual(summary.reviewDecisionFinalizationBoundaryStatus, ORO10H_STATUS);
  ...
  assertNoSensitiveShape("ORO-10H happy output", JSON.stringify(summary));
}

function assertHeld(summary, blocker) {
  assert.strictEqual(summary[ORO10H_RESULT_KEY], HOLD);
  assert.strictEqual(summary.result, HOLD);
  assert(summary.blockers.includes(blocker), `missing blocker ${blocker}.`);
  assertClosedFlags(summary);
  assertNoSensitiveShape("ORO-10H hold output", JSON.stringify(summary));
}

function runOro10hDetailedSmoke(options = {}) {
  assertDocsAndRegistration();
  assertProtectedRuntimeFilesUntouched();
  assertNoLongOro10hFilenames();
  assertStaticSafety();
  assert.strictEqual(typeof helper.ORO10H_PHASE, "string");
  assert.strictEqual(typeof helper.ORO10H_STATUS, "string");
  assert.strictEqual(typeof buildOro10hReviewDecisionFinalizationBoundaryRecord, "function");
  assert.strictEqual(typeof validateOro10hReviewDecisionFinalizationBoundary, "function");
  assert.strictEqual(typeof buildOro10hSafetySummary, "function");

  const happy = evaluateOro10hReviewDecisionFinalizationBoundary(
    fixtures.validStaticReviewDecisionFinalizationBoundaryFromOro10gFixture
  );
  assertHappyPath(happy);
  assertHappyPath(buildOro10hReviewDecisionFinalizationBoundaryRecordResult());
  assertHappyPath(
    validateOro10hReviewDecisionFinalizationBoundary(buildOro10hReviewDecisionFinalizationBoundaryRecord())
  );
  const safety = buildOro10hSafetySummary();
  ...
  const heldCases = [
    ...
    [fixtures.missingStaticReviewDecisionFinalizationRecordFailFixture, "missing_static_review_decision_finalization_record"],
  ...
  for (const [input, blocker] of heldCases) assertHeld(evaluateOro10hReviewDecisionFinalizationBoundary(input), blocker);
  const allReports = fixtures
    .buildOro10hApprovalChainRolloverReviewDecisionFinalizationBoundaryFixtures()
    .map(evaluateOro10hReviewDecisionFinalizationBoundary);
  assert(allReports.length >= 34, "fixture set must cover requested ORO-10H scenarios.");
