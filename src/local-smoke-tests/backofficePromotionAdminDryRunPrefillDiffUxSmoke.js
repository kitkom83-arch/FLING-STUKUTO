...
const UI_MARKER = "BACKOFFICE-PROMOTION-ADMIN-DRY-RUN-PREFILL-DIFF-UX-49";
...
function createUiHarness() {
  ...
  const fetchCalls = [];
  const promotionId = "local-smoke-promo-49";
...
async function main() {
  ...
  assertIncludes("package.json", read("package.json"), [
    "smoke:backoffice-promotion-admin-dry-run-prefill-diff-ux",
    "backofficePromotionAdminDryRunPrefillDiffUxSmoke.js",
  ]);
  ...
  assertNoSecretsOnSources([
    ...
    ["src/local-smoke-tests/backofficePromotionAdminDryRunPrefillDiffUxSmoke.js", read(__filename)],
  ...
  assert.strictEqual(getElement(harness, "admin-promotion-dry-run-promotion-id").value, "local-smoke-promo-49");
  ...
  setValue(harness, "admin-promotion-dry-run-audit-reason", "prefill diff ux smoke");
  setChecked(harness, "admin-promotion-dry-run-acknowledgement", true);

  assert.strictEqual(diffPreview.textContent.includes("title: Summer Bonus - Summer Bonus Prefill UX"), true);
  ...
  await waitFor(() => harness.fetchCalls.some((call) => call.pathname === "/api/admin/promotions/local-smoke-promo-49/dry-run"), "dry-run request");
  const dryRunCall = harness.fetchCalls.find((call) => call.pathname === "/api/admin/promotions/local-smoke-promo-49/dry-run");
  ...
  assert.strictEqual(dryRunCall.body.after.title, "Summer Bonus Prefill UX");
  assert.strictEqual(dryRunCall.body.after.bonusValue, 300);
  assert.strictEqual(dryRunCall.body.after.maxWithdraw, 1200);
  assert.strictEqual(dryRunCall.body.auditReason, "prefill diff ux smoke");
  ...
  const noSelectionCalls = harness.fetchCalls.filter((call) => call.pathname === "/api/admin/promotions/local-smoke-promo-49/dry-run").length;
  ...
  assert.strictEqual(selectedSummary.textContent.includes("id local-smoke-promo-49"), true);
  assert.strictEqual(diffPreview.textContent.includes("title: Summer Bonus"), true);

  console.log("Backoffice promotion admin dry-run prefill diff UX package/docs wiring: PASS");
  console.log("Backoffice promotion admin dry-run prefill diff UX source contract: PASS");
  console.log("Backoffice promotion admin dry-run prefill diff UX prefill behavior: PASS");
  console.log("Backoffice promotion admin dry-run prefill diff UX diff preview: PASS");
  console.log("Backoffice promotion admin dry-run prefill diff UX fail-closed behavior: PASS");
  console.log("Backoffice promotion admin dry-run prefill diff UX safety flags: PASS");
  console.log("Backoffice promotion admin dry-run prefill diff UX secret scan: PASS");
  console.log("Backoffice promotion admin dry-run prefill diff UX smoke: PASS");
}

main().catch((error) => {
  console.error("Backoffice promotion admin dry-run prefill diff UX smoke: FAIL");
  console.error(error);
  process.exitCode = 1;
});
